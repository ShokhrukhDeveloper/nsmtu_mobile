...
class ImageSlideShowWidget extends StatelessWidget {
  const ImageSlideShowWidget({Key? key, required this.controller})
      : super(key: key);
    ...
    return Container(
      ...
      child: Obx(
        () => ImageSlideshow(children: [
          if (!controller.showImageSlide.value) const SizedBox(),
          if (controller.showImageSlide.value)
            ...controller.imageSlides
                .map((e) => image(
                    url: e.sliderImg ?? '',
                    text: e.title ?? "",
                    context: context))
